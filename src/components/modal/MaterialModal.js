...
const MaterialModal = ({ show, setShow, modalType, obraID, itemID }) => {
  ...
  async function getMaterial() {
    let cliente;
    try {
      const res = await axios.get("/material?material_id=" + itemID);
      if (res.status === 200) {
  ...
  async function onDelete(setAlert) {
    try {
      let res = await axios.delete("/material?material_id=" + fields._id);
      if (res.status === 204) {
        setShow(false);
      }
    } catch (e) {
  ...
  async function onSubmit(setAlert, clearFields, event) {
    ...
    try {
      let res;
      if (modalType === "novo")
        res = await axios.post("/material/material", fields, config);
      else res = await axios.put("/material", fields, config);
    ...
    <div>
      <Modal show={show} onHide={onClose} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>
            {modalType === "novo" ? "Novo " : "Editar "}Material
          </Modal.Title>
        </Modal.Header>
