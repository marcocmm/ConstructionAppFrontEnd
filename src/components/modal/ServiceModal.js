...
const ServiceModal = ({ show, setShow, modalType, obraID, itemID }) => {
  ...
  async function getService() {
    let cliente;
    try {
      const res = await axios.get("/service?service_id=" + itemID);
      if (res.status === 200) {
  ...
  async function onDelete(setAlert) {
    try {
      let res = await axios.delete("/service?service_id=" + fields._id);
      if (res.status === 204) {
        setShow(false);
      }
    } catch (e) {
  ...
  async function onSubmit(setAlert, clearFields, event) {
    ...
    try {
      let res;
      if (modalType === "novo")
        res = await axios.post("/service/service", fields, config);
      else res = await axios.put("/service", fields, config);
    ...
    <div>
      <Modal show={show} onHide={onClose} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>
            {modalType === "novo" ? "Novo " : "Editar "}Serviço
          </Modal.Title>
        </Modal.Header>
